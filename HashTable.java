...
public class HashTable<K, V>
{
	...
	*/
	public int findKey(K key)
	{
		...
		//displacement is used to increment through the hash table, away from the original location which key.hashCode() points to
		int displacement = 0;
		while(table[(Math.abs(key.hashCode()) + displacement) % table.length] != null && !((K) table[(Math.abs(key.hashCode()) + displacement) % table.length].key).equals(key) && displacement < table.length)
			displacement++;
		if (displacement == table.length || table[(Math.abs(key.hashCode()) + displacement) % table.length] == null )
			return -1;
		return displacement;
	}
