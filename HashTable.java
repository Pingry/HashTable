...
public class HashTable<K, V>
{
	...
	*/
	...
	/**
	* Create a new Entry object which contains both the key and the value. Puts the Entry in the hash table based off the key's hash code. 
	...
	*/
	public void put(K key, V value)
	{
		...
		{
			if (table[Math.abs(input.key.hashCode()) % table.length] == null)
				occupied++;
		...
		//displacement is used to increment through the hash table, away from the original location which key.hashCode() points to
			int displacement = 0;
			while (table[(Math.abs(input.key.hashCode()) + displacement) % table.length] != null && !((K) table[(Math.abs(input.key.hashCode()) + displacement) % table.length].key).equals(key))
				displacement += 1;
			if (table[(Math.abs(input.key.hashCode()) + displacement) % table.length] == null)
				occupied++;
	...
	* @return	the string which contains a visualization of the hash table
	*/
	...
	*/
	...
	{
		int keyLocation = findKey(key);
		if (keyLocation == -1)
			return null;
		occupied--;
		V output = (V) table[(Math.abs(key.hashCode()) + keyLocation) % table.length].value;
		table[(Math.abs(key.hashCode()) + keyLocation) % table.length] = null;
		int displacement = keyLocation + 1;;
